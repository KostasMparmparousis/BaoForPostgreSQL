...
def save_final_model_and_history(parent_dir):
    """Save the final trained model and all history, then clean original repo"""
    # Configuration
    model_dir = Path(parent_dir).parent.joinpath("models/bao/random/final_model")
...
def save_model_checkpoint(c_idx, parent_dir):
    """Save Bao model checkpoint after retraining iteration"""
    checkpoint_dir = Path(parent_dir).parent.joinpath("models/bao/random/checkpoints")
...
def pg_connection_string(db_name):
    return f"dbname={db_name} user=suite_user host=train.darelab.athenarc.gr port=5469 password=71Vgfi4mUNPm"
...
def main(args):
    ...
    for c_idx, chunk in enumerate(bao_chunks):
        ...
        if USE_BAO:
            ...
            os.system("sync")
            save_model_checkpoint(c_idx + 1, parent_dir)
    ...
    save_final_model_and_history(parent_dir)
